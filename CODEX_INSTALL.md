...
9. 打开招聘 JD 发布助手侧栏，选择“飞书文档”，让使用者用自己的飞书账号点击“授权飞书”。粘贴一份有效公司与岗位语料并解析，然后点击“检查并生成写入计划”；该动作会自动验证授权、文档读取、模板识别、重复岗位和最新文档版本。正式文档固定为：

   <https://zhenfund.feishu.cn/wiki/RTWjwVZjri4uCUk0J8wcn2K3n6d>

   计划生成后停止，不要点击确认写入。只有部分完成、结果未知或结构校验失败时，界面才会显示“打开正式文档检查”。
...
- 飞书授权和“检查并生成写入计划”是否由使用者完成，以及失败时的原始错误。
