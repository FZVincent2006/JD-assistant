...
3. 预览总岗位数、已正确数量以及待更新的公司和岗位。
4. 勾选要补链的公司。首次建议只选 `CoFANCY 可糖`，确认两个岗位跳转正确后再处理其他公司。
5. 点击“确认补全已选 N 个岗位链接”，并在确认框中再次确认。
6. 扩展按公司名、岗位名、地点和招聘类型唯一匹配岗位 JD 标题，原位更新所选公司的 Portfolio Bullet 后回读校验。
7. 继续勾选其他公司补链；全部完成后再次检查应显示“全部岗位链接已正确”。

如果检查后提示文档版本已变化，扩展不会写入；重新点击“检查岗位链接”生成最新计划。
公司或岗位无法唯一匹配、Portfolio 岗位字段不完整、富文本无法安全保留或待更新项超过
单批 200 个时，整批停止，不猜测也不部分写入。
...
混合链接或无法确认目标的链接会停止计划并提示人工检查，不会被自动覆盖。
